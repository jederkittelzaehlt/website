...
<?php
        ...
        $items = $pages->find("template=bedarf"); // Update the selector to choose the pages you want to use
	...
	$lines[] = "---";
                ...
                if ($page->ansprechpartner) {
                    $lines[] = "Ansprechpartner: {$page->ansprechpartner}";
                }
				 if ($page->telefonnummer) {
                    $lines[] = "Telefonnummer: {$page->telefonnummer}";
                }

                // Here's the geocoded address for this location...
                if ($page->location->address) {
                    $lines[] = "Addresse: {$page->location->address}";
                }
				if ($page->anzahl) {
                    $lines[] = "Anzahl benötigte Kittel: {$page->anzahl}";
                }
